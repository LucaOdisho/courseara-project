...
def get_scatter_chart(entered_site, slider):
    filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site][spacex_df["Payload Mass (kg)"] < slider[1]][spacex_df["Payload Mass (kg)"] > slider[0]]
    if entered_site == 'ALL':
        fig = px.scatter(spacex_df[spacex_df["Payload Mass (kg)"]< slider[1]][spacex_df["Payload Mass (kg)"]> slider[0]], x='Payload Mass (kg)',
                         y='class',
                         color="Booster Version Category",
                         title='Payload mass versus class'
                         )
        return fig
    else:
